...
const BACKTEST_SYSTEM =
  "\n\nYou are currently reviewing the user's BACKTESTING journal. The user manually backtests strategies on TradingView and logs each trade here with a rule checklist, price/SL/target/BE, risk (% or $), a 'did the plan change?' note, psychology notes, and an outcome. Focus on: (1) did the trade follow the strategy rules, (2) edge and consistency across the backtest, (3) process and psychology. Be direct, concise, high-impact, and use Markdown.";
...
export function AiCoachChat({
  ...
  async function streamReply(history: ChatMessage[]) {
    ...
    const baseSystem = `${BASE_SYSTEM}${backtest ? BACKTEST_SYSTEM : ""}`;
  ...
  const quickPrompts = [
    {
      ...
      text: "Based on my backtesting data, which setup is my most profitable and what mistakes are costing me the most R?",
    },
    {
      label: "Review Last Session",
      icon: <Target size={14} />,
      text: "Review my last 5 backtested trades. What patterns do you see in my rule adherence and execution?",
    },
    {
      label: "Risk Check",
      icon: <TrendingUp size={14} />,
      text: "Am I risking too much per backtested trade? What does my risk consistency look like?",
    },
  ];

  const busy = isThinking || isStreaming;
  const subtitle = backtest
    ? "Backtesting Strategy Mentor"
    : busy
      ? "Generating response…"
      : "Elite Neural Trading Mentorship";
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <p className="px-4 text-sm leading-relaxed text-[#A0A0C0]">
                    {backtest
                      ? "Let's sharpen your backtested edge. What are we reviewing today?"
                      : "Your mental edge is as important as your technical edge. What are we optimizing today?"}
                  </p>
                </div>
                ...
                {loadingContext ? (
                  ...
                  <div className="flex items-center gap-2 rounded-full border border-[#1E1E38] bg-[#111120] px-3 py-1.5 text-[11px] text-[#8080A0]">
                    ...
                    <span>
                      {tradingContext.tradeCount} trades loaded ·{" "}
                      {tradingContext.winRate.toFixed(0)}% WR ·{" "}
        ...
        <div className="shrink-0 border-t border-[#1E1E38] bg-[#0D0D1A]/70 px-4 pt-3 pb-20 lg:pb-3 backdrop-blur-md sm:px-6 lg:px-10">
          <form onSubmit={handleSend} className="relative mx-auto max-w-3xl">
            <textarea
              ...
              placeholder="Describe a backtested trade, an emotion, or a rule-break..."
              rows={1}
