...
const navLinks = [
  ...
  { label: "Backtesting", href: "#backtesting-os" },
  { label: "Pricing", href: "#pricing" },
  { label: "FAQ", href: "#faq" },
];


function LoginLink({ variant = "nav" }: { variant?: "nav" | "mobile" }) {
  const base =
    "rounded-lg border border-line font-semibold text-txt transition-colors hover:border-gold hover:text-gold";
  const cls = variant === "nav" ? `${base} px-4 py-2 text-sm` : `${base} px-3 py-3 text-center`;
  return (
    <motion.a
      href="/login"
...
const heroStats = [
  { v: "AI", l: "Daily Plan" },
  { v: "QR", l: "Backtest Reports" },
  { v: "CSV", l: "Broker Import" },
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-2 lg:gap-8">
        <motion.div variants={heroContainer} initial="hidden" animate="show" className="text-center lg:text-left">
          ...
          >
            ...
            AI Trading OS · Built for Prop Traders
          </motion.div>

          <motion.h1 variants={heroItem} className="text-4xl leading-[1.08] font-bold tracking-tight sm:text-5xl lg:text-6xl">
            Plan the session.
            <br />
            <span className="text-shimmer">Protect your edge.</span>
          </motion.h1>

          <motion.p variants={heroItem} className="mx-auto mt-6 max-w-xl text-base text-muted2 sm:text-lg lg:mx-0">
            The AI trading operating system for serious prop traders. Plan before you trade,
            log every execution, find leaks, and share verified backtesting reports.
          </motion.p>
...
const featuresData = [
  {
    ...
    desc: "Your dashboard tells you what needs attention today: Daily Plan status, review queue, biggest leak, prop firm risk, and weekly review readiness.",
    accent: "from-gold/20 to-transparent",
    glow: "group-hover:shadow-[0_0_50px_-10px_rgba(240,180,41,0.4)]",
  },
  {
    icon: "✅",
    title: "Daily Plan + Execution Score",
    desc: "Generate max trades, risk per trade, allowed setups, avoid list, and stop rules — then score whether you actually followed the plan.",
  ...
  },
  {
    icon: "🛡️",
    title: "AI Risk-Guard",
    desc: "Clock in with stress, sleep, discipline, targets, and loss limits. Clock out with a clean behavioral report and next-session prescription.",
    accent: "from-purple2/20 to-transparent",
    glow: "group-hover:shadow-[0_0_50px_-10px_rgba(168,85,247,0.4)]",
  },
  {
    icon: "🧠",
    title: "AI Trade Review + Leak Finder",
    desc: "AI reviews trades, suggests emotion, mistake, lesson, and next rule — while Leak Finder ranks exactly what is costing you R.",
    accent: "from-blue2/20 to-transparent",
    glow: "group-hover:shadow-[0_0_50px_-10px_rgba(76,130,251,0.4)]",
  },
  {
    icon: "📄",
    title: "Backtesting Reports + PDF QR",
    desc: "Create public backtesting reports with win rate, expectancy, drawdown, rule adherence, downloadable PDFs, and QR verification.",
    accent: "from-gold/20 to-transparent",
    glow: "group-hover:shadow-[0_0_50px_-10px_rgba(240,180,41,0.4)]",
  },
  {
    icon: "📥",
    title: "Import Center + Prop Firm Mode",
    desc: "Import CSVs from MT4, MT5, TradingView, Tradovate, NinjaTrader, TopstepX and track prop-firm drawdown buffers with templates.",
...
/* =====================================================================
   7. AI TRADING OS WORKFLOW
   ===================================================================== */

const workflowData = [
  { step: "01", title: "Plan", text: "Open Daily Plan before the market and lock max trades, risk, setups, avoid list, and stop rules." },
  { step: "02", title: "Execute", text: "Log trades or import broker CSVs. ProfitPnL tracks setup, session, emotion, risk, and R-multiple." },
  { step: "03", title: "Review", text: "AI Trade Review fills emotion, mistake, lesson, notes, and reviewed status so your data becomes useful." },
  { step: "04", title: "Improve", text: "Weekly Review, Leak Finder, and Risk-Guard turn patterns into rules for the next session." },
];

function TradingOSWorkflow() {
  return (
    <section id="ai-workflow" className="relative overflow-hidden border-y border-line/60 bg-ink2/45 py-20 sm:py-28">
      <div className="animate-pulse-glow absolute top-1/2 -right-40 h-96 w-96 rounded-full bg-gold/8 blur-[130px]" />
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          ...
          className="mx-auto mb-14 max-w-2xl text-center"
        >
          <p className="mb-3 font-mono2 text-xs tracking-[0.3em] text-gold uppercase">{"//"} AI Workflow</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
            One loop that makes traders <span className="text-shimmer">more disciplined</span>
          </h2>
          <p className="mt-4 text-muted2">
            ProfitPnL is designed around the real trading cycle: plan, execute, review, improve.
          </p>
        </motion.div>
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {workflowData.map((item, i) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="group relative overflow-hidden rounded-2xl border border-line bg-panel/70 p-6 backdrop-blur transition-all duration-300 hover:border-gold/40 hover:shadow-[0_0_55px_-20px_rgba(240,180,41,0.55)]"
            >
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-gold/10 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
              <div className="relative">
                <p className="font-mono2 text-xs font-bold text-gold">{item.step}</p>
                <h3 className="mt-3 text-xl font-black text-txt">{item.title}</h3>
                <p className="mt-3 text-sm leading-relaxed text-muted2">{item.text}</p>
              </div>
            </motion.div>
          ))}
        </div>
...
/* =====================================================================
   9. BACKTESTING REPORTS PREVIEW
   ===================================================================== */

function BacktestingReportsPreview() {
  return (
    <section id="backtesting-os" className="relative overflow-hidden py-20 sm:py-28">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <motion.div
          ...
          transition={{ duration: 0.7 }}
        >
          <p className="mb-3 font-mono2 text-xs tracking-[0.3em] text-gold uppercase">{"//"} Backtesting Reports</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
            Share backtesting results with <span className="text-shimmer">PDF + QR verification</span>
          </h2>
          <p className="mt-4 text-muted2">
            Build reports from selected backtest models, export CSV, download a PDF, and let anyone scan a QR code to verify the public report page.
          </p>
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <a href="/register" className="gold-gradient rounded-xl px-6 py-3 text-center text-sm font-bold text-ink">Create Free Account</a>
            <a href="#pricing" className="rounded-xl border border-line px-6 py-3 text-center text-sm font-bold text-txt hover:border-gold/50 hover:text-gold">View Pricing</a>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: "-60px" }}
          transition={{ duration: 0.7 }}
          className="rounded-2xl border border-line bg-panel/70 p-5 backdrop-blur"
        >
          <div className="mb-4 flex items-center justify-between border-b border-line/60 pb-4">
            <div>
              <p className="font-mono2 text-[10px] tracking-[0.22em] text-gold uppercase">Verified Report</p>
              <h3 className="mt-1 text-xl font-black">London ORB Backtest</h3>
            </div>
            <div className="flex h-20 w-20 items-center justify-center rounded-xl bg-white text-ink text-3xl">▦</div>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <div key={label} className="rounded-xl border border-line bg-ink2/70 p-3">
                <p className="text-[9px] tracking-wider text-dim uppercase">{label}</p>
                <p className="mt-1 font-mono2 text-lg font-black text-gold">{value}</p>
              </div>
            ))}
          </div>
          <div className="mt-4 rounded-xl border border-gold/25 bg-gold/8 p-3 text-xs leading-6 text-gold">
            Includes public report link, downloadable PDF, QR code, trade sample, model quality, drawdown, expectancy, and rule adherence.
          </div>
        </motion.div>
      </div>
    </section>
...
const plansData = [
  {
    ...
    features: [
      ...
      { t: "Limited CSV import", ok: true, bold: false },
      { t: "AI Co-Pilot reports", ok: false, bold: false },
      { t: "AI Risk-Guard", ok: false, bold: false },
    ],
  ...
  },
  {
    ...
    features: [
      ...
      { t: "AI Coach + saved AI reports", ok: true, bold: false },
      { t: "Daily Plan + Weekly Review", ok: true, bold: false },
      { t: "Backtesting PDFs + QR reports", ok: true, bold: false },
      { t: "AI Risk-Guard + Leak Finder", ok: true, bold: false },
    ],
...
const footerCols = [
  {
    ...
    items: [
      ...
      { label: "Backtesting Reports", href: "#backtesting-os" },
    ],
  },
  {
    h: "Resources",
